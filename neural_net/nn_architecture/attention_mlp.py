import torch
import torch.nn as nn
import math


class LearnedPositionalEncoding(nn.Module):
    def __init__(self, d_model, max_len=558):
        super(LearnedPositionalEncoding, self).__init__()
        self.pos_embedding = nn.Parameter(torch.zeros(1, max_len, d_model))

    def forward(self, x):
        return x + self.pos_embedding


class ATT_MLP(nn.Module):
    def __init__(self, seq_length, feature_dim, d_model, nhead, num_encoder_layers, dim_feedforward, output_dim, dropout=0):
        super(ATT_MLP, self).__init__()

        # Encoders
        self.encoder1 = nn.TransformerEncoder(
            nn.TransformerEncoderLayer(d_model=d_model, nhead=nhead, dim_feedforward=dim_feedforward, dropout=dropout,
                                       batch_first=True, activation=nn.GELU()),
            num_layers=num_encoder_layers
        )

        # Linear layers to adapt dimensions
        self.input_adapter1 = nn.Linear(feature_dim, d_model)

        # Positional encoding is the same for both
        self.learned_encoding = LearnedPositionalEncoding(d_model, max_len=feature_dim)

        # Output layer
        self.output_linear = nn.Linear(d_model * seq_length, output_dim)  # Adjust output layer size

    def forward(self, src1):
        # Prepare inputs
        src1 = src1.unsqueeze(2)
        src1 = self.input_adapter1(src1)  # N, 60, 25 -> N, 60, d_model

        # Add positional encoding
        src1 = self.learned_encoding(src1)

        # Encode with transformers
        encoded1 = self.encoder1(src1)
        encoded1 = encoded1.view(encoded1.size(0), -1)
        output = self.output_linear(encoded1)
        return output


# Testing the model
def model_test():
    N = 64  # Batch size
    src1 = torch.randn(N, 558)  # N, Layers, Features

    model = ATT_MLP(
        seq_length=558,
        feature_dim=1,
        d_model=32,
        nhead=1,
        num_encoder_layers=8,
        dim_feedforward=256,
        output_dim=368,
    )

    output = model(src1)
    print("Output Shape:", output.shape)


#model_test()
