...
from seq2seq_utils import seq2scalar_32, count_parameters, eval_model, collate_fn, seq2scalar_flat
...
model_name = f'seq2scalar_weighted_32_positional_{min_std}_flat.model'

#model = torch.load(f"models/{model_name}")

seq_length = len(FEAT_COLS)
model = ModifiedSequenceToScalarTransformer_positional(1, output_dim, d_model, nhead, num_encoder_layers, dim_feedforward, dropout, seq_length).cuda()
...
val_dataset, _ = seq2scalar_flat(True, val_data, FEAT_COLS, TARGET_COLS, mean_x, std_x, mean_y, std_y)
...
while patience < num_epochs:
    ...
    for idx, df in enumerate(batches):
        ...
        train_dataset, _ = seq2scalar_flat(True, df, FEAT_COLS, TARGET_COLS, mean_x, std_x, mean_y, std_y)
